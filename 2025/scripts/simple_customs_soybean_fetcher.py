...
使用requests和BeautifulSoup进行数据获取和解析。
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('china_customs_soybeans_simple.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SimpleCustomsDataFetcher:
    """简化版中国海关数据获取器"""
    ...
    def _extract_date_from_text(self, text):
        ...
        # 尝试匹配 YYYY-MM-DD 格式
        year_month_day_match = re.search(r'(\d{4})-(\d{1,2})-(\d{1,2})', text)
        if year_month_day_match:
            year, month, day = year_month_day_match.groups()
            return f"{year}-{month.zfill(2)}-{day.zfill(2)}"
        ...
        # 尝试匹配 YYYY-MM 格式
        year_month_match = re.search(r'(\d{4})-(\d{1,2})(?!-\d{1,2})', text)
        if year_month_match:
            year, month = year_month_match.groups()
            return f"{year}-{month.zfill(2)}"
